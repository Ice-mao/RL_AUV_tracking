...
class WorldAuvV2SampleCustom(WorldBase):
    """
    ...
    def reset(self):
        ...
        if METADATA['eval_fixed']:
            ...
            self.agent_init_pos = np.array([50.00000000, 0.00000000, self.fix_depth])  # 西南角落
            self.agent_init_yaw = -0.785  # 45度（朝东北）
            self.target_init_pos = np.array([54.09731175, -4.53808490, self.fix_depth])
            self.target_init_yaw = -0.8973672591142523  # -45度（朝西北）
        else:
