...
def train_sac(args: argparse.Namespace = get_args()) -> None:
    # env
    env = TeachObsWrapper(auv_env.make(args.env,
                                       render=args.render,
                                       record=args.record,
    ...
    train_envs = DummyVectorEnv([lambda: TeachObsWrapper(auv_env.make(args.env,
                                                                      render=args.render,
                                                                      record=args.record,
                                                                      num_targets=args.nb_targets,
                                                                      is_training=False,
    ...
    test_envs = DummyVectorEnv([lambda: TeachObsWrapper(auv_env.make(args.env,
                                                                     render=args.render,
                                                                     record=args.record,
                                                                     num_targets=args.nb_targets,
                                                                     is_training=False,
    ...
    actor = ActorProb(net, args.action_shape, device=args.device, unbounded=True, preprocess_net_output_dim=256).to(args.device)
