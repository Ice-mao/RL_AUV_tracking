...
def policy_to_callable(
    ...
    if policy is None:
        ...
        raise TypeError(
            "Policy must be None, a stable-baselines policy or algorithms, "
    ...
    *,
    ...
    Args:
        policy: Can be any of the following:
            1) A stable_baselines3 policy or algorithms trained on the gym environment.
    ...
    wrapped_obs = types.maybe_wrap_in_dictobs(obs['images'])
    ...
    while np.any(active):
        ...
        acts, state = get_actions(obs['state'], state, dones)
        obs, rews, dones, infos = venv.step(acts)
        assert isinstance(
            obs,
            (np.ndarray, dict),
        ), "Tuple observations are not supported."
        # TODO if you want to collect different obs:
        wrapped_obs = types.maybe_wrap_in_dictobs(obs['images'])
...
def generate_transitions(
    ...
    **kwargs: Any,
    ...
    Args:
        policy: Can be any of the following:
            - A stable_baselines3 policy or algorithms trained on the gym environment
