...
class DRQPolicy(DDPGPolicy[TSACTrainingStats], Generic[TSACTrainingStats]):  # type: ignore[type-var]
    ...
    def update(
            ...
            buffer1: ReplayBuffer | None,
            buffer2: ReplayBuffer | None,
            **kwargs: Any,
    ) -> TTrainingStats:
        ...
        batch1, indices1 = buffer1.sample(round(sample_size / 2))
        batch2, indices2 = buffer2.sample(round(sample_size / 2))
        self.updating = True
        batch = self.process_fn(batch1, buffer1, indices1)
        with torch_train_mode(self):
            training_stat = self.learn(batch1, **kwargs)
            self.learn(batch2, **kwargs)
        self.post_process_fn(batch1, buffer1, indices1)
