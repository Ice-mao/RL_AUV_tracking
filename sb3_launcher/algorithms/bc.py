...
class TanhNormal(Normal):
    ...
    def log_prob(self, actions):
        ...
        # 2. 原始高斯分布的对数概率
        log_prob_parent = super().log_prob(gaussian_actions)

        # 3. Jacobian 修正项（正确符号）
        log_jacobian = th.log(1 - actions.pow(2) + self.epsilon)
        # log_jacobian = log_jacobian.sum(-1)  # 沿动作维度求和

        # 4. 最终对数概率
        log_prob = log_prob_parent + log_jacobian  # 注意是加号！
        return log_prob

    @staticmethod
    def inverse_tanh(x):
        # 数值稳定的逆 tanh 实现
        return 0.5 * (th.log1p(x + 1e-6) - th.log1p(-x + 1e-6))
...
class BC(algo_base.DemonstrationAlgorithm):
    ...
    def __init__(
            ...
            *,
            ...
            rng: np.random.Generator,
            policy: Optional[Actor] = None, # TODO
    ...
    def train(
        ...
        def process_batch():
            th.nn.utils.clip_grad_norm_(self.policy.parameters(), max_norm=1.0)  # TODO
            self.optimizer.step()
